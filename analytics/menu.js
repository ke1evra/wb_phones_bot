        ...
        for (let manager in messageData) {
            if (!(manager === "all_managers" || manager === "null")) {
                ...
                if (messageData[manager]['orders']) {
                    ...
                    for (let status in messageData[manager]['orders']) {
                        message += `${messageData[manager]['orders'][status]['count'] + ' '.repeat(ordersShift - String(String(messageData[manager]['orders'][status]['count']).length))}${orderStatusIcons[status]} ${status} (${(messageData[manager]['orders'][status]['sum']).toLocaleString('ru-RU')} ₽)\n`
                    }
                    message += `\n`
                }
                if (messageData[manager]['calls']) {
